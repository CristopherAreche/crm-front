...
function SideBar({typeSidebar}) {
  ...
  return (
    <>
        <section className={`${isOpen ? 'left-0' : '-left-full'} col-span-2 bg-base-light/70 lg:bg-base-light/40 w-68 lg:w-80  flex flex-col gap-y-8 justify-between py-4 px-12 fixed lg:static  h-full z-50 transition-all duration-200 lg:left-0 border-r border-light/40`}>
            <section className="flex flex-col gap-y-4">
                <h1 className="text-4xl font-bold tracking-widest text-center border-b border-light/40 text-white pb-8">LOGO</h1>
                {/* Contenido Dinamico */}
                {
        ...
        <button className="absolute bottom-6 right-4 text-4xl lg:hidden hover:bg-base-light/60 rounded-full py-1 px-2 box-content z-50 text-light" onClick={() => setIsOpen(!isOpen)}>
